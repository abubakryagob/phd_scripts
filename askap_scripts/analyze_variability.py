...
Usage:
    python analyze_variability.py <path_to_fits_files> <source_ra> <source_dec> <aperture_radius>
...
def plot_light_curve(time_indices, fluxes, errors, metrics, output_file=None):
    """
    ...
    ax.set_title('ZTF J1901+1458 Variability Analysis')
...
def main():
    ...
    if len(sys.argv) != 5:
        print("Usage: python analyze_variability.py <path_to_fits_files> <source_ra> <source_dec> <aperture_radius>")
        sys.exit(1)
    ...
    fits_files = sorted(glob.glob(fits_pattern), key=extract_time_index)
    ...
    time_indices = [extract_time_index(f) for f in fits_files]
    ...
    for i, fits_file in enumerate(fits_files):
        print("Processing {} ({}/{})...".format(os.path.basename(fits_file), i+1, len(fits_files)))
    ...
    results_file = "variability_results.txt"
    ...
    # Plot the light curve
    light_curve_file = "ZTF_J1901_light_curve.png"
    plot_light_curve(time_indices, fluxes, errors, metrics, light_curve_file)
    
    # Save the flux measurements to a CSV file
    data_file = "ZTF_J1901_flux_data.csv"
